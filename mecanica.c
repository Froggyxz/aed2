...
void retirar_peca() {
        ...
        scanf("%d", &id);
        printf("Digite o nome do funcionario que esta retirando a peca: ");
        ...
        for(i = 0; i < count; i++) {
            if(pecas[i].id == id) {
                ...
                count--;
                Retirar retirada;
                strcpy(retirada.nome_peca, pecas[i].nome);
                retirada.id_peca = pecas[i].id;
                retirada.qtd_peca = 1;
                strcpy(retirada.data_retirada, "dd/mm/aaaa");
                strcpy(retirada.nome_funcionario, nome_funcionario);

                char filename[100];
                sprintf(filename, "Retirada-%s_%s_%d.txt", retirada.data_retirada, retirada.nome_funcionario, retirada.id_peca);

                struct stat st = {0};
                if (stat("Histórico de retirada", &st) == -1) {
                mkdir("Histórico de retirada", 0700);
                }

                FILE *file = fopen(filename, "w");
                if (file != NULL) {
                    fprintf(file, "Nome da peça: %s\n", retirada.nome_peca);
                    fprintf(file, "ID da peça: %d\n", retirada.id_peca);
                    fprintf(file, "Quantidade retirada: %d\n", retirada.qtd_peca);
                    fprintf(file, "Data da retirada: %s\n", retirada.data_retirada);
                    fprintf(file, "Nome do funcionário: %s\n", retirada.nome_funcionario);
                    fclose(file);
                }
                return;
                    }
